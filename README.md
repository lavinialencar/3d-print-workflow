...
| Command-line slicing with Orca + G-code validation | ✅ Verified, with a fix | root cause confirmed on a real cube: the CLI does not walk a machine profile's `inherits` chain (30 missing keys, including the bed size). `scripts/fix_bambu_machine_profile.py` patches just those keys; after the fix, 0 of the settings that shape the print (walls, layers, infill, shells, supports, seam, speeds) differed from the GUI. See [docs/06](docs/06-slicing-with-an-assistant.md#pitfall-the-command-line-does-not-resolve-profile-inheritance-like-the-gui-and-the-fix-that-closes-it) |
