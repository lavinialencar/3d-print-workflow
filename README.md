...
| Search for an existing model first (Printables) | ✅ Verified live | `scripts/find_models.py` lists likes, downloads, makes and licence; unofficial endpoint, can change; MakerWorld, Thingiverse and Thangs are read through a real browser (Thingiverse also has an official API that needs your own token; Cults3D shows a bot check and is left alone), see [docs/08](docs/08-modeling-and-delivery.md#0-look-for-it-before-you-model-it) |
