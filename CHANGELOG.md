...
- 24 tests for the analyzer, built on shapes with known answers (cube, sphere, cylinder, T bracket, thin fin, thick block, wide plate). They also check that every rule has a source and a trust level, that unverified rules are never applied, and that every setting written exists in OrcaSlicer.
